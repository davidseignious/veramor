...
async function profileFromModal(){const id=document.getElementById('profileModalBody')?.dataset.profileId;if(!id)return null;const {data}=await promptSb.from('profiles').select('id,prompts').eq('id',id).maybeSingle();return data||null}
...
async function decorateDeck(){const card=document.querySelector('#deck .profile-card');if(!card||card.querySelector('.vera-prompt-profile-section')||!card.dataset.profileId)return;const id=card.dataset.profileId;const {data:p}=await promptSb.from('profiles').select('id,prompts').eq('id',id).maybeSingle();if(!p?.prompts?.length||!card.isConnected||card.dataset.profileId!==id)return;const details=card.querySelector('.details');if(!details)return;const sec=document.createElement('section');sec.className='vera-prompt-profile-section';const mini=document.createElement('div');mini.className='vera-prompt-mini';mini.innerHTML=await answerHtml(p.prompts[0],true);sec.appendChild(mini);details.insertBefore(sec,details.querySelector('.actions'))}
