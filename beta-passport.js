...
async function call(action,extra={}){
  ...
  if(!r.ok)throw new Error(data.error||'Profile Passport unavailable');
  return data;
}
...
async function createPassport(){
  ...
  try{
    ...
    status(data.beta_perks?'Share link ready. Paid Profile Passport is unlocked for friend beta testing.':'Share link ready.','ok');
  }catch(e){status(e.message,'bad')}
  finally{b.disabled=false;b.textContent='Create share link'}
}
async function sharePassport(){
  if(!passportUrl)return status('Create a share link first.','warn');
  try{
...
async function revokePassport(){
  if(!confirm('Revoke your current Profile Passport link? Anyone with that link will no longer be able to open it.'))return;
  const b=document.getElementById('passportRevoke');b.disabled=true;b.textContent='Revoking…';
  try{await call('revoke');passportUrl='';document.getElementById('passportShare').disabled=true;document.getElementById('passportCopy').disabled=true;status('Profile Passport revoked.','ok')}
  catch(e){status(e.message,'bad')}
