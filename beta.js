...
async function renderChat(){clearInterval(chatTimer);$('#matchModalBody').innerHTML=`<div class="notice ok">✓ Both Chemistry Checks complete · messaging unlocked</div><div class="chat" id="chatMessages"></div><div class="compose"><input class="input" id="chatInput" maxlength="2000" placeholder="Message ${esc(activeMatch.other.display_name)}…"><button class="btn primary" id="sendMessage">Send</button></div><div class="actions" style="margin-top:12px"><button class="btn" id="chatSafety">Block / report</button><button class="btn danger" id="unmatchBtn">Unmatch</button></div><div id="chatMsg"></div>`;$('#sendMessage').onclick=sendMessage;$('#chatInput').onkeydown=e=>{if(e.key==='Enter'){e.preventDefault();sendMessage()}};$('#chatSafety').onclick=()=>openSafety(activeMatch.other,{surface:'conversation',match_id:activeMatch.id});$('#unmatchBtn').onclick=unmatchActive;await loadMessages();chatTimer=setInterval(()=>{if(!$('#matchModal').classList.contains('hidden'))loadMessages(false).catch(()=>{});else clearInterval(chatTimer)},5000)}
async function loadMessages(scroll=true){if(!activeMatch)return;const {data,error}=await sb.from('messages').select('*').eq('match_id',activeMatch.id).order('created_at',{ascending:true});if(error)throw error;const root=$('#chatMessages');if(!root)return;root.innerHTML=(data||[]).map(x=>`<div class="msg ${x.sender_id===user.id?'me':''}" data-message-id="${esc(x.id)}">${esc(x.body)}${x.sender_id===user.id&&x.read_at?'<div style="font-size:10px;opacity:.7;margin-top:4px">Seen</div>':''}${x.sender_id!==user.id?`<button class="msg-report" type="button" data-report-message="${esc(x.id)}">Report message</button>`:''}</div>`).join('');$$('[data-report-message]').forEach(b=>b.onclick=()=>{const row=(data||[]).find(x=>x.id===b.dataset.reportMessage);if(row)openSafety(activeMatch.other,{surface:'message',match_id:activeMatch.id,message_id:row.id,message_excerpt:row.body})});await sb.rpc('mark_match_messages_read',{p_match:activeMatch.id});if(scroll)root.scrollTop=root.scrollHeight}
